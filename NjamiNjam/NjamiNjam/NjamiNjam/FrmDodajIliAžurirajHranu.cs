...
namespace NjamiNjam
{
    public partial class FrmDodajIliAžurirajHranu : Form
    {
        Artikli odabrani;
        Klase.Hrana hrana = new Klase.Hrana();
        public FrmDodajIliAžurirajHranu()
        {
            InitializeComponent();
        }

        public FrmDodajIliAžurirajHranu(Artikli artikli)
        {
            InitializeComponent();
            odabrani = artikli;
        }

        private void FrmDodajIliAžurirajHranu_Load(object sender, EventArgs e)
        {
            if (odabrani != null)
            {
        ...
        private void btnUredu_Click(object sender, EventArgs e)
        {
            ...
            else if (odabrani == null)
            {
                ...
                hrana.DodajNovuHranu(txtNaziv.Text, decimal.Parse(txtCijena.Text), int.Parse(txtKolicina.Text), DateTime.Now, ID);

            }
            else
            {
                hrana.AzurirajHranu(odabrani, txtNaziv.Text, decimal.Parse(txtCijena.Text), int.Parse(txtKolicina.Text));
